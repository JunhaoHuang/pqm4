    ...
    do // move the racc_api do-while here.
    {
        ...
        do
        {
            ...
            for (i = 0; i < RACC_ELL; i++)
            {
                ...
                polyr_ntt_smul(vz[i], z, -MONT_RRR1, -MONT_RRR2); // negative & Montgomery domain
#endif
            ...
            for (i = 0; i < RACC_K; i++)
            {
                ...
                expand_aij(aij, i, 0, seed);
#if RACC_D == 32
                l_z = racc_decode_sig_z(z, CRYPTO_BYTES, l_z, &pre_zz, &pre_kz, sig);
                polyr_fntt(z);
                polyr_ntt_cmul(y, z, aij);
#else

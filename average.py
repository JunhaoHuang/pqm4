...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Summarize cycle counts in Dawn/speed_*.txt files."
    )
    parser.add_argument(
        "paths",
        nargs="*",
        help="Files or glob patterns to summarize. Defaults to Dawn/speed_*.txt.",
    )
    return parser
...
def resolve_paths(patterns: list[str]) -> list[Path]:
    if not patterns:
        return sorted(Path(".").glob("Dawn/speed_*.txt"))

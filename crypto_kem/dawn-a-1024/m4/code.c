...
static inline uint32_t div193_u24(uint32_t x, uint32_t *r)
{
    uint32_t q = ((uint64_t)x * 11126859u) >> 31;

    *r = x - q * 193u;
    return q;
}

static inline uint16_t divmod193_u48(uint64_t *x)
{
    uint64_t v = *x;
    uint32_t r = 0;
    uint32_t q2, q1, q0;

    q2 = div193_u24((uint32_t)(v >> 32), &r);
    q1 = div193_u24((r << 16) | ((uint32_t)(v >> 16) & 0xffffu), &r);
    q0 = div193_u24((r << 16) | ((uint32_t)v & 0xffffu), &r);
...
void decode_pk(uint8_t *code_c, int16_t *c)
{
    ...
    for(i = 0; i < 4; i++)
    {
        c[DIM_N-4+i]=divmod769_u48(&tmp[204]);
...
void decode_ct(uint8_t *code_c, int16_t *c)
{
    ...
    for(i = 0; i < 4; i++)
    {
        c[DIM_N - 4 + i] = divmod193_u48(&tmp[204]);
    }
